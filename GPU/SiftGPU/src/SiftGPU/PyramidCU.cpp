...
void PyramidCU::ResizePyramid(int w, int h)
{
	...
	if(GlobalUtil::_verbose && GlobalUtil::_timingS) std::cout<<"[Allocate Pyramid]:\t" <<(totalkb/1024)<<"MB\n";
        ...
        //std::cout<<"Punto cuatro"<<std::endl;
...
{
	//std::cout<<"Estas la entrada a ConvertInputTOCU"<<std::endl; 
	int ws = input->GetImgWidth(), hs = input->GetImgHeight();
	TruncateWidth(ws);
	//copy the input image to pixel buffer object
    if(input->_pixel_data)
    {
	//std::cout<<"Estas la entrada a ConvertInputTOCU.if"<<std::endl; 
        _inputTex->InitTexture(ws, hs, 1);
        _inputTex->CopyFromHost(input->_pixel_data); 
    }else
    {
	//std::cout<<"Estas la entrada a ConvertInputTOCU.else.E"<<std::endl; 
        //glutCreateWindow("test"); 
	if(_bufferPBO == 0) glGenBuffers(1, &_bufferPBO);
	//std::cout<<"Estas la entrada a ConvertInputTOCU.else.S"<<std::endl; 
	//std::cout<<input->_rgb_converted<<std::endl; 
	//std::cout<<input->CopyToPBO(_bufferPBO, ws, hs, GL_LUMINANCE)<<std::endl;
	//std::cout<<"Estas la entrada a ConvertInputTOCU.else.S.2"<<std::endl; 
	if(input->_rgb_converted && input->CopyToPBO(_bufferPBO, ws, hs, GL_LUMINANCE))
        {
	//std::cout<<"Estas la entrada a ConvertInputTOCU.else.if"<<std::endl; 
		    _inputTex->InitTexture(ws, hs, 1);
            _inputTex->CopyFromPBO(ws, hs, _bufferPBO); 
        }else if(input->CopyToPBO(_bufferPBO, ws, hs))
	    {
	//std::cout<<"Estas la entrada a ConvertInputTOCU.else.else"<<std::endl; 
...
void PyramidCU::BuildPyramid(GLTexInput * input)
{
	...
	//std::cout<<"Estas la entrada a BuildPyramid-1"<<std::endl; 
	USE_TIMING();
	...
	//std::cout<<"Estas la entrada a BuildPyramid-2"<<std::endl; 
	...
	//std::cout<<"Estas la entrada a BuildPyramid-2.1.E"<<std::endl; 
		OCTAVE_START();

	//std::cout<<"Estas la entrada a BuildPyramid-2.1.S"<<std::endl; 
		if( i == _octave_min )
		{	
	//std::cout<<"Estas la entrada a BuildPyramid-2.1.2.if"<<std::endl; 
			ConvertInputToCU(input);

	//std::cout<<"Estas la entrada a BuildPyramid-2.1.2.if.1"<<std::endl; 
			if(i == 0)
			{
	//std::cout<<"Estas la entrada a BuildPyramid-2.1.2.if.if"<<std::endl; 
			ProgramCU::FilterImage(tex, _inputTex, buf,param.GetInitialSmoothSigma(_octave_min + _down_sample_factor));
			}else
			{
	//std::cout<<"Estas la entrada a BuildPyramid-2.1.2.if.else"<<std::endl; 
		...
		{
	//std::cout<<"Estas la entrada a BuildPyramid-2.1.2.else"<<std::endl; 
	...
	//std::cout<<"Estas la entrada a BuildPyramid-2.2"<<std::endl; 
		LEVEL_FINISH();
	...
	//std::cout<<"Estas la entrada a BuildPyramid-3"<<std::endl; 
	if(GlobalUtil::_timingS) ProgramCU::FinishCUDA();

	//std::cout<<"Estas la entrada a BuildPyramid-4"<<std::endl; 
	if(ProgramCU::CheckErrorCUDA("PyramidCU::BuildPyramid")) SetFailStatus();
	//std::cout<<"Estas la entrada a BuildPyramid-5"<<std::endl; 
}
