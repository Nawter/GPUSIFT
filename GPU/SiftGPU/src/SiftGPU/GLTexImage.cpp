...
int GLTexInput::LoadImageFile(char *imagepath, int &w, int &h )
{
	...
	{
        fclose(file);
        std::cerr << "First ERROR: fileformat not supported\n";
        return 0;
	}
	else
        {
	...
	{
        std::cerr << "Second ERROR: fileformat not supported\n";
	done = 0;
	}
    if(done)    SetImageData(w, h, data, GL_LUMINANCE, GL_UNSIGNED_BYTE);
