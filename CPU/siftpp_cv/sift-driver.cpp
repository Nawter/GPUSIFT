...
//new line 28/06/2014
void draw(const cv::Mat3b& image, cv::Mat3b& display_image, VL::m_locations locations)
{
    std::cout << "keypoints.size-3::::" <<locations.size() << std::endl;	    
    std::vector<KeyPoint> keypoints(locations.size());
    foreach_idx(i, keypoints)
            keypoints[i] = (const KeyPoint&)locations[i];
    cv::Mat dummy;
  ...
  while( argc > 0 ) {
	...
	for( VL::Sift::KeypointsConstIter iter = sift.keypointsBegin() ;
	       iter != sift.keypointsEnd() ; ++iter ) {
            //std::cout << "keypoints.size-1:::::"<<locations.size()<< std::endl;	    
	    // detect orientations
	    VL::float_t angles [4] ;
	    int nangles ;
	    if( ! noorient ) {
	 ...
	 // new line 28/06/2014   
		 ...
		 new_location.angle = angles[a];	       
	      ...
	      out << endl ;
            ...
            std::cout << "keypoints.size-2::::"<<locations.size()<< std::endl;	    
	}
   ...
   // new line 29/06/2014  
   std::cout << "keypoints.size-4::::"<<locations.size()<< std::endl;	    
   cv::Mat3b image = imread("data/person_and_bike_004.png");
   cv::Mat3b display_image2;
   draw(image, display_image2,locations);
   imwrite("data/debug_features_1.png", display_image2);
   return 0 ;
}
